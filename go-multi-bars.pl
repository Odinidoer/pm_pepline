...
my $usage = <<"USAGE";
       ...
       Usage :perl $0 -i GO.list -u up-protein-list -d down-protein-list -w 20 -h 25
       input file:
                   -i        STRING   GO annotation file for all protein( GO.list )
...
while(<FIN1>){
	chomp $_;
	#if (exists $protein2go{$_}){print OUT1 $_."\t".$protein2go{$_}}else{next;}
        if ($protein2go{$_} ne ""){print OUT1 $_."\t".$protein2go{$_}."\n"}else{next;}
	
}
while(<FIN2>){
	chomp;
 	if ($protein2go{$_} ne ""){print OUT2 $_."\t".$protein2go{$_}."\n"}else{next;}
}
close FIN1;
close FIN2;
close OUT1;
...
open (SH,">gene-ontology.sh");
print SH '/mnt/ilustre/users/ting.kuang/scripts/proteomics/pipeline/bin/gene-ontology.pl -i '.$upgos.' -l 2 -list '.$uplevel2.' > '.$uplevel2.'.txt'."\n";
print SH '/mnt/ilustre/users/ting.kuang/scripts/proteomics/pipeline/bin/gene-ontology.pl -i '.$downgos.' -l 2 -list '.$downlevel2.' > '.$downlevel2.'.txt'."\n";
close SH;
system("sh gene-ontology.sh");
...
pdfname <-paste(\"$upprotein\",\"-\",\"$downprotein\",\"-\",\"gobars.pdf\",sep=\"\")
par(mar = c(md,ml,mu,mr)+0.1)
pdf(file=pdfname,width=$opts{w},height=$opts{h})
Bars<- barplot(barplot_matrix[,ncol(barplot_matrix):1],beside=TRUE,horiz=T,col=mycol_up_down,axes=FALSE,asp=NA,border=NA,plot=TRUE,main=Titlename,xlim=c(-(2*MN)+$opts{lws},MN+round(MN/2)),xlab=paste(SpaceLength,\"Number of Proteins ( Up/Down )\",collapse=\"\"))
pos_Y<-Bars[1,]+0.5
...
#names(out_mat)<-c(\"GO term\",\"num of up protein\",\"num of down protein\")
#write.table(out_mat,paste(\"$uplevel2\",\"-\",\"$downlevel2\",\"-\",\"gobars.xls\",sep=\"\"),sep=\"\t\",col.names=T,row.names=F,quote=F)
#### out

#### out
data1<-read.delim(\"$uplevel2\",header=T,check.names=F)
data2<-read.delim(\"$downlevel2\",header=T,check.names=F)
all <- unique(merge(x = data1, y = data2, by = \"Term\", all = TRUE))
all <- all[-which(all\$Term == \"Term\"),]
newdf <- subset(all,select = c(\"Term\",\"Number_of_protein.x\",\"Protein_GO_list.x\",\"Number_of_protein.y\",\"Protein_GO_list.y\"))
colnames(newdf)<-c(\"GO term\",\"num of up protein\",\"up proteins\",\"num of down protein\",\"down proteins\")
write.table(newdf,paste(\"$upprotein\",\"-\",\"$downprotein\",\"-\",\"gobars.xls\",sep=\"\"),na = '0',sep=\"\t\",col.names=T,row.names=F,quote=F)
...
## axes: number of protein, percent of protein
...
system ("rm $opts{o}.r");
system ("rm gene-ontology*");
system ("rm *level2");
system ("rm $opts{u}GO.list");
system ("rm $opts{d}GO.list");
