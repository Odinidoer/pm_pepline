...
my $usage = <<"USAGE";
       ...
       Usage :perl $0 [options]
			...
			-db	/state/partition1/COG/cog.db		database name for sqlite,defualt:	/state/partition1/COG/cog.db or /mnt/ilustre/app/rna/database/COG/cog.db
...
print TABLE "#Qeury_name\tQeury_length\tHsp_start_of_query\tHsp_end_of_query\tHsp_strand_of_query\tHit_name\tHit_description\tHit_length\tHsp_start_of_hit\tHsp_end_of_hit\tCOG/KOG/NOG_group\tCOG/KOG/NOG_group_description\tCOG/KOG/NOG_group_categeries\tCOG/KOG/NOG_region_start\tCOG/KOG/NOG_region_end\tCoverage_of_COG/KOG/NOG_region\tIdentities_of_COG/KOG/NOG_region\tPositives_Identities_of_COG/KOG/NOG_region\n";
foreach my $f (@file){
...
open(LIST, "> $opts{o}/cog.list.xls") || die "Can't open $opts{o}/cog.list.xls\n";
print LIST "Query_name\tCOG\tKOG\tNOG\n";
foreach my $q (@seqs){
	print LIST "$q";
	foreach my $t (qw/COG KOG NOG/){
...
foreach my $q (keys(%cogs)){
	foreach my $t (qw/COG KOG NOG/){
		foreach my $c (keys(%{$cogs{$q}{$t}})){
			...
			if($cat){
				foreach my $aa (@$cat){
					$sumary{$general->{$aa}{type}}{$aa}{$t}{$q}=1;
				}
			}
		}
	}
}
open(SUMARY, "> $opts{o}/cog.sumary.xls") || die "Can't open $opts{o}/cog.sumary.xls\n";
print SUMARY "#Total seqs with COG/KOG/NOGs:".scalar(keys(%lists))."\n";
print SUMARY "#Type\tfunctional_categories\tCOG\tKOG\tNOG\n";
...
foreach my $t (@types){
	foreach my $c (sort keys %{$sumary{$t}}){
		...
		foreach my $z (qw/COG KOG NOG/){			
			print SUMARY "\t".scalar(keys %{$sumary{$t}{$c}{$z}});
		}
		print SUMARY "\n";
	}
}
