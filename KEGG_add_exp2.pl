...
my %opts;
my $VERSION="2.0";
GetOptions( \%opts,"i=s", "format=s","o=s","up=f","down=f","maxEvalue=f","minIdentity=i","log=f","org=s","fresh!","exp=s","exptype=s","rank=i","database=s","QminCoverage=i","HminCoverage=i","use_proxy!","proxy_server=s","server=i","parse_id!", "h!");
...
my $usage = <<"USAGE";
       ...
       Usage :perl $0 [options]
			...
			-format		blastformat		the format of blast output
						kobas(default)	kobas2 anntation file			wise       Genewise -genesf format
			-exp		expressfile
			-line
			-type		1|2 (1 only diffexpression gene; 2 gene with logFC > 1)
			-o dir		output dir (defualt: pathways)                 
			-org		organism name of three letters ,list in http://www.genome.jp/kegg/catalog/org_list.html ,like hsa (default:ko)
						also can use:map
			-up			default: 1.2
			-down		default: 0.8 (have not considered the p value)
			-fresh		fresh database from network
			-database	database path		default:/mnt/ilustre/app/rna/database/kegg/kegg.db
...
$opts{up}=$opts{up}?$opts{up}:1.2;
$opts{down}=$opts{down}?$opts{down}:0.8;
...
my $local_img_dir = "/mnt/ilustre/users/ting.kuang/scripts/proteomics/pipeline/bin/kegg";
...
unless($opts{format} eq 'kobas'){
	...
	foreach my $f (@file){
		...
		while(my $result = $searchio->next_result){
			...
			$proteinkos{$query_name}=&uniq(\@quiery_ko);
		}
	}
}else{
...
#		my $line=<ANNOT>;
		while(<ANNOT>){
			...
			if(/^(\S*)\s+(.*)$/){
				...
				my @kos=split /,\s/,$ko; ###20160524
				$proteinkos{$protein}=\@kos;	###20160524		
				
				if(exists $expression{$protein}){					
					my $fc;
					if ( exists $expression{$protein}{fc} ){
						$fc = $expression{$protein}{fc};
					}else{
						$fc = $expression{$protein}{count1}/$expression{$protein}{count2};
					}
					if ($fc eq "noTest"){
						$fc = 0 ;
					}
				...
				if (exists $ko_exps{$ko}{up} || exists $ko_exps{$ko}{down}){
					if(exists($kos->{$koid})){
						foreach my $p (keys(%{$kos->{$koid}})){
							...
							push(@{$pathway->{$p}{'proteins'}},$protein);					
						}
					}else{
						warn("warn:$ko is not in all pathways in this organism $opts{org},if your database is newest, this is ok ... \n");
					}
...
print PATHWATY "Pathway\tPathway_definition\tNumber_of_proteins\tProteins_ko_list\tPathway_image_name\n";
...
foreach my $p (keys(%$pathway)){
	...
	my $proteinlist=&uniq(\@{$pathway->{$p}{'proteins'}});
	my $protein_ko_list;
	foreach my $n (@$proteinlist){
		$protein_ko_list.=$n."(".join(",",@{$proteinkos{$n}}).");";
	}
	print PATHWATY "$p\t".$pathway->{$p}{'definition'}."\t".scalar(@$proteinlist)."\t".$protein_ko_list."\t".$imgname."\n";
}
...
";
	...
	system("matlab -nojvm -nodisplay <$opts{o}/pic.m");
