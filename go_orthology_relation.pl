...
system("sed 's/example/$opts{enrich}.temp/g' /mnt/ilustre/users/ting.kuang/scripts/proteomics/pipeline/bin/GO_picture.R >$opts{enrich}.temp.R");

system("R --no-save --restore < $opts{enrich}.temp.R");
