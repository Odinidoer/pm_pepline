...
$opts{pc}=defined$opts{pc}?$opts{pc}:0.6;
$opts{lc}=defined$opts{lc}?$opts{lc}:1;
$opts{sc}=defined$opts{sc}?$opts{sc}:0.8;
$opts{mb}=defined$opts{mb}?$opts{mb}:12;
...
###		}
	...
	close PWD_1;
	## add pathway class
	system "perl /mnt/ilustre/users/ting.kuang/scripts/proteomics/tabletools_add.pl -i /mnt/lustre-2/users/wangyan/rna/database/KEGG/pathwayName_typeII_typeI -t $opts{i}.pathway.xls -n 1 -headt T -headi T > $opts{i}.pathway_addclass.xls";
	system "perl /mnt/ilustre/users/ting.kuang/scripts/proteomics/tabletools_add.pl -i /mnt/lustre-2/users/wangyan/rna/database/KEGG/pathwayName_typeII_typeI -t $opts{i}.disease.xls -n 1 -headt T -headi T > $opts{i}.disease_addclass.xls";
}
...
if(type==\"PATHWAY\"){
	...
	if(nrow(PATHWAY_enrichraw)==0){
		...
		#text(0,length(continue_cols)+500,\"Pvalue\",adj=0,xpd=T)  #pvalue
		dev.off()
...
if(type==\"DISEASE\"){
	...
	if(nrow(PATHWAY_enrichraw)==0){
		...
		text(rep(0.6,5),Bars[c(200,2501,5001,7500,9800)],as.character(c(1,0.75,0.5,0.25,0)),cex=1,adj=0,xpd=T)
		#text(0,length(continue_cols)+500,\"Pvalue\",adj=0,xpd=T)   #pvalue 
		text(0,length(continue_cols)+500,\"FDR\",adj=0,xpd=T)    #fdr
		dev.off()
