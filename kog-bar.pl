...
kog <-matrix(c(\"A\" , \"RNA processing and modification\",
...
kog[1,2]
for (i in 1:25){
     if(length(which(dat[,1] \%in\% kog[i,2]))){
         x[i] <- dat[ which(dat[,1] \%in\% kog[i,2]),2]
     }else{ x[i] <- 0 }
...
row.names(x) <- kog[,1]
x
pdf(\"KOG.class.catalog.pdf\",width=$opts{w},height=$opts{h})

#layout(matrix(c(1,2),1,2),width=c(5:6.2))
par(mar=c(3,3,2,0))
xx <-barplot(x,beside=TRUE,col=rainbow(25),space=0.4,las=1,xlim=c(-4,70),ylim=c(-0.3*max(x),max(x)*1.15),cex.names=0.6,main=\"KOG Function Classification\",cex.main=0.8,cex.lab=0.8,font.lab=3,font.names=3,axes=FALSE,adj.lab=0)
...
#plot.new()
#par(mar=c(4,0,2,0))
legend(x=38,y=max(x)*1.1,legend=paste(row.names(x),\": \",kog[,2]),cex=0.6,bty=\"n\",y.intersp=1.5)
