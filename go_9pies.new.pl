...
my $goid;
GetOptions(
	\%opts,"i=s","o=s","l1=i","l2=i","l3=i","pie=s","pw=f","ph=f","pc1=f","pc2=f","d=s","h=s","p=s","u=s","port=i","help!");

my $usage = <<"USAGE";
		Program: $0
		Version: v1.0 2016-01-25
		Usage :perl $0 [options]
				-i*	[GO files from goa-extract]
					##########################################################
					# F9UKV8	GO:0003824;GO:0008152;GO:0016787;GO:0016787
					# F9UKV9	GO:0008152;GO:0016787
					# F9UKX5	GO:0005524;GO:0046872
					# F9UKY2	GO:0003333;GO:0016020;GO:0016021
					##########################################################
				-o	output prefix for the Rscript, defaults: GO.list
				-l1	GO level,2
				-l2	GO level,3
				-l3	GO level,4 
		...
		Mysql database options :
				...
				-u	[Mysql database username:Default "blast2go"]
				-p	[Mysql database password:Default "blast4it"]
				-d	[Mysql GO database name:Default "b2g"]
				-port	[Mysql database port:Default "3306"]
				
		-help	Display this usage information
		*	must be given Argument 
		example:go_9pies.pl -i GO.list -o prefix 
USAGE
...
while (<FILE>) {
	...
	if ( $protein[0] =~ /^GO:\d+$/ ) {
		...
		for ( my $n = 1 ; $n <= $#protein ; $n++ ) {
			if($protein[$n]=~/\;/){
				my @b=split(/;\s*/,$protein[$n]);
				push( @{ $golist{ $protein[0] }{'GO'} }, @b);
			}else{
				push( @{ $golist{ $protein[0] }{'GO'} }, $protein[$n] );
			}
		}
	}
}
...
foreach $protname ( keys(%golist) ) {
	foreach $goid ( @{ $golist{$protname}{'GO'} } ) {
		my $sth = $dbh->prepare(
"SELECT DISTINCT ancestor.*, graph_path.term1_id AS ancestor_id FROM term INNER JOIN graph_path ON (term.id=graph_path.term2_id) INNER JOIN term AS ancestor ON (ancestor.id=graph_path.term1_id)  WHERE term.acc='$goid';"
		...
		foreach my $id ( keys(%$ref) ) {
			...
			#unless ( exists( $protlist{$termname}{$protname} ) ) {
				#$terms{$termname}++;
				#$golist{$protname}{$termname} = $ref->{$id}->{'acc'};
				$protlist{$termname}{$protname}{$goid}=1;
			#}
		}
		$sth->finish;
	}
...
open(OUT1,">$opts{o}.level$opts{l1}.txt");
	...
	foreach my $key ( keys(%bcm) ) {
		for ( my $i = 1 ; $i<= $x ; $i++ ) {
		 ...
		 foreach my $lterm (keys( %{ $bcm{$key}{$opts{l1}} } )){
		   if (exists($protlist{$lterm})){
		   	$zterms{$lterm}=scalar(keys %{$protlist{$lterm}});
		   }		   
	     }
	    #if($opts{list}){
	    	open LIST1, ">> $opts{o}.level$opts{l1}.xls" or die "Error:Cannot open file $opts{o}.level$opts{l1} : $! \n";
			print LIST1 "Term_type\tTerm\tGO_id\tNumber_of_protein\tProtein_GO_list\n";
	    	foreach my $nterm (keys(%zterms)){
	    		my @proteins;
	    		foreach my $x (keys %{$protlist{$nterm}}){
	    			push(@proteins,$x."(".join(",",keys %{$protlist{$nterm}{$x}}).")");
	    		}
	    		print LIST1 "$key\t$nterm\t".$bcm{$key}{$opts{l1}}{$nterm}."\t".$zterms{$nterm}."\t".join(";",@proteins)."\n";
	    	}
	    	close LIST1;
	    #}
...
### level2
open(OUT2,">$opts{o}.level$opts{l2}.txt");
print OUT2 "total proteins number with gos: $sum \n";
	...
	foreach my $key ( keys(%bcm) ) {
	    ...
	    #if($opts{list}){
	    	open LIST2, ">> $opts{o}.level$opts{l2}.xls" or die "Error:Cannot open file $opts{o}.level$opts{l2} : $! \n";
			print LIST2 "Term_type\tTerm\tGO_id\tNumber_of_protein\tProtein_GO_list\n";
	    	foreach my $nterm (keys(%zterms)){
	    		my @proteins;
	    		foreach my $y (keys %{$protlist{$nterm}}){
	    			push(@proteins,$y."(".join(",",keys %{$protlist{$nterm}{$y}}).")");
	    		}
	    		print LIST2 "$key\t$nterm\t".$bcm{$key}{$opts{l2}}{$nterm}."\t".$zterms{$nterm}."\t".join(";",@proteins)."\n";
	    	}
	    	close LIST2;
	    #}
...
### level3
open(OUT3,">$opts{o}.level$opts{l3}.txt");
print OUT3 "total proteins number with gos: $sum \n";
	...
	foreach my $key ( keys(%bcm) ) {
	    ...
	    #if($opts{list}){
	    	open LIST3, ">> $opts{o}.level$opts{l3}.xls" or die "Error:Cannot open file $opts{o}.level$opts{l3} : $! \n";
			print LIST3 "Term_type\tTerm\tGO_id\tNumber_of_protein\tProtein_GO_list\n";
	    	foreach my $nterm (keys(%zterms)){
	    		my @proteins;
	    		foreach my $z (keys %{$protlist{$nterm}}){
	    			push(@proteins,$z."(".join(",",keys %{$protlist{$nterm}{$z}}).")");
	    		}
	    		print LIST3 "$key\t$nterm\t".$bcm{$key}{$opts{l3}}{$nterm}."\t".$zterms{$nterm}."\t".join(";",@proteins)."\n";
	    	}
	    	close LIST3;
	    #}
...
################# ploting pies ###################
plot_pie<-\"$opts{pie}\"
Level_1<-paste(\"$opts{o}\",\".level\",$opts{l1},\".txt\",sep=\"\")
Level_2<-paste(\"$opts{o}\",\".level\",$opts{l2},\".txt\",sep=\"\")
Level_3<-paste(\"$opts{o}\",\".level\",$opts{l3},\".txt\",sep=\"\")
pdfname<-paste(\"$opts{o}\",\".Level\",\"$opts{l1}\",\"$opts{l2}\",\"$opts{l3}\",\".pdf\",sep=\"\")
if(plot_pie==\"T\"){
