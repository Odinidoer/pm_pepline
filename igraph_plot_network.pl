...
my $usage = <<"USAGE";
	Program:	$0   
	Discription:	plot network for different expressed proteins from STRING database
	Version:	$VERSION
	...
	Usage:	perl $0 [options]		
		...
		-iExp*	input different express file, e.g. *_vs_*.DE.exp.xls (no head)
			##################################################################
			#Q61315	3.90E+09	6.20E+09	1.575655114	0.655951786	0.04679188	yes	up
			#Q02248	3.90E+06	2.30E+06	0.605968858	-0.722684442	0.017789102	yes	down
			#Q3USK2	1.80E+07	3.00E+07	1.640144665	0.71382307	0.031204267	yes	up
			##################################################################
...
#根据log2(FC)值的4分位设置点的大小,大小分别为：3,4,5,6,7,9
count <- nodes\$V5
count <- abs(count)
...
#根据p值设置点边框颜色，P值越小，颜色越深
frame_color <- nodes\$V8
names(frame_color) <- nodes\$V1
color_list <- c(\"#000000\", \"#333333\", \"#666666\", \"#999999\", \"#CCCCCC\")
frame_color[which(frame_color < 0.001)] <- color_list[1]
frame_color[which(frame_color < 0.005 & frame_color >= 0.001)] <- color_list[2]
frame_color[which(frame_color < 0.01 & frame_color >= 0.005)] <- color_list[3]
frame_color[which(frame_color < 0.05 & frame_color >= 0.01)] <- color_list[4]
frame_color[which(frame_color >= 0.05)] <- color_list[5]
...
plot(net, layout = layout_with_fr,
     ...
     vertex.frame.color=V(net)\$frame.color, 
     vertex.label.cex=1,
     vertex.label.font=1,
